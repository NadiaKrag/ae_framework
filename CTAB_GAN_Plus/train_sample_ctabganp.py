...
def train_ctabgan(
    ...
    if len(meta_data) == X_num_train.shape[1]:
        print('LATENT', real_data_path)
        X = pd.DataFrame(X_num_train)
    else:   
    ...
    if meta:
        ...
        if ds_name != None:
            ctabgan_params = lib.load_json("CTAB_GAN_Plus/columns.json")[ds_name]
        else:
            try:
                ctabgan_params = lib.load_json("CTAB_GAN_Plus/columns.json")[real_data_path.name]
            except:
                ctabgan_params = lib.load_json("CTAB_GAN_Plus/columns.json")['latent']
        
    print(ctabgan_params)
    
    train_params["batch_size_ctab"] = min(y_train.shape[0], train_params["batch_size_ctab"])

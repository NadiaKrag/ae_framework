...
from CTAB_GAN_Plus.model.synthesizer.transformer import ImageTransformer,DataTransformer
from CTAB_GAN_Plus.model.privacy_utils.rdp_accountant import compute_rdp, get_privacy_spent
...
class CTABGANSynthesizer:
    def __init__(self,
        ...
        self.micro_batch_size = 1 #batch_size
        self.clip_coeff = 1
        self.sigma = sigma #1.02 noise multiplier
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, general=[], non_categorical=[], type={}):
        ...
        self.transformer = DataTransformer(train_data=train_data, categorical_list=categorical, mixed_dict=mixed, general_list=general, non_categorical_list=non_categorical, meta=self.meta)
        ...
        sides = [4, 8, 16, 24, 32, 64, 128]
        col_size_g = data_dim
        print('col size g', col_size_g)
        for i in sides:
            if i * i >= col_size_g:
                self.gside = i
                break
            
        print(self.dside, self.num_channels)
        ...
        for i in tqdm(range(self.epochs)):
            ...
            if elapsed_time > 20 * 60:
                print("Training stopped as it exceeded 20 minutes.")
                break'''
            ...
            for _ in range(ci):
                ...
                if self.private:
                    ...
                    print('PRIVATE TRAINING', self.micro_batch_size)
    ...
    def sample(self, n, seed=0):
        ...
        t0 = time.time()
        while len(result) < n and (time.time() - t0) <= 300:
            print('SAMPLER', len(result), n, time.time()-t0)
            data_resample = []    

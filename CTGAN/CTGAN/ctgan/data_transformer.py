...
import numpy as np
import pandas as pd
from rdt.transformers import ClusterBasedNormalizer, OneHotEncoder #BayesGMMTransformer, OneHotEncodingTransformer
...
class LogTransformer(BaseEstimator, TransformerMixin):
    def fit(self, X, y=None):
        # Store any information needed for the transformation
        return self

    def transform(self, X):
        # Apply log transformation to each element
        return np.log1p(X)
...
class DataTransformer(object):
    ...
    def __init__(self, ae=None, max_clusters=10, weight_threshold=0.005):
        ...
        self.rec = False #minmax=true, gm=false
        self.count = 0
        self.ae = ae
    ...
    def fit(self, raw_data, discrete_columns=()):
        ...
        """
        self.output_info_list = [] #change to work with stadlers metadata
        ...
        for column_name in raw_data.columns: #0,...,d,y
        ...
        for column_transform_info in self._column_transform_info_list:
            ...
            if column_transform_info.column_type == 'continuous':
                '''if not self.rec:
                    ...
                    column_data_list.append(column_transform_info.transform.transform(data))'''
                if self.ae:
                    column_data_list.append(column_transform_info.transform.transform(data))
                else:
                    column_data_list.append(self._transform_continuous(column_transform_info, data))
        ...
        for column_transform_info in self._column_transform_info_list:
            ...
            if column_transform_info.column_type == 'continuous':
                '''if not self.rec:
                    ...
                    recovered_column_data = column_transform_info.transform.inverse_transform(column_data)'''
                if self.ae:
                    recovered_column_data = column_transform_info.transform.inverse_transform(column_data)
                else:
                    recovered_column_data = self._inverse_transform_continuous(column_transform_info, column_data, sigmas, st)

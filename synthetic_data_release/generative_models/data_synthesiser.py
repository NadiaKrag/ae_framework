...
from synthetic_data_release.generative_models.data_synthesiser_utils.datatypes.FloatAttribute import FloatAttribute
from synthetic_data_release.generative_models.data_synthesiser_utils.datatypes.IntegerAttribute import IntegerAttribute
from synthetic_data_release.generative_models.data_synthesiser_utils.datatypes.StringAttribute import StringAttribute
from synthetic_data_release.generative_models.data_synthesiser_utils.utils import bayes_worker, normalize_given_distribution, exponential_mechanism
...
from synthetic_data_release.utils.logging import LOGGER
...
class BayesianNet(GenerativeModel):
    ...
    """
    def __init__(self, metadata, histogram_bins=10, degree=1, infer_ranges=False, multiprocess=False, seed=None):
        self.metadata = self._read_meta(metadata)
        self.histogram_bins = histogram_bins
        self.degree = degree
    ...
    def generate_samples(self, nsamples):
        print('Generating samples', self.__name__)
        LOGGER.debug(f'Generate synthetic dataset of size {nsamples}')
...
class PrivBayes(BayesianNet):
    ...
    """
    def __init__(self, metadata, histogram_bins=10, degree=1, epsilon=.1, infer_ranges=False, multiprocess=False, seed=None):

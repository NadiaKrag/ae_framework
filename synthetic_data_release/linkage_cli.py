...
from CTAB_GAN_Plus.model.ctabgan import CTABGAN
...
from synthetic_data_release.attack_models.stadler_helper import run_AEframework
...
def main():
    ...
    if 'generativeModels' in runconfig.keys():
        for gm, paramsList in runconfig['generativeModels'].items():
            if gm == 'IndependentHistogram':
                ...
                print(paramsList)
                gmList.append(BayesianNet(metadata, paramsList[0][1]['histogram_bins'], paramsList[0][1]['degree']))
            elif gm == 'PrivBayes':
                #for params in paramsList:
                    #gmList.append(PrivBayes(metadata, *params))
                gmList.append(PrivBayes(metadata, paramsList[0][1]['histogram_bins'], paramsList[0][1]['degree'], paramsList[0][1]['epsilon']))
            elif gm == 'CTGAN':
                for params in paramsList:
                ...
                gmList.append(CTABGAN(df=None, metadata=metadata))
            elif gm == 'DPCTABGAN':
                gmList.append(CTABGAN(df=None, metadata=metadata, private=True))
            elif gm == 'DDPM':
                gmList.append(GaussianMultinomialDiffusion([0], 0, 0, meta=metadata))
            elif gm == 'TVAE':
                gmList.append(TVAESynthesizer(meta=metadata, name=gm))
            elif 'AE-BayesianNet' in gm:
                gmList.append(TVAESynthesizer(meta=metadata, name=gm))
            elif 'AE-PrivBayes' in gm:
                gmList.append(TVAESynthesizer(meta=metadata, name=gm))
            elif 'AE-CTABGAN' in gm:
                gmList.append(TVAESynthesizer(meta=metadata, name=gm))
            elif 'AE-DPCTABGAN' in gm:
                gmList.append(TVAESynthesizer(meta=metadata, name=gm))
            elif 'AE-DDPM' in gm:
                gmList.append(TVAESynthesizer(meta=metadata, name=gm))
            else:
    ...
    for tid in targetIDs:
        ...
        for GenModel in gmList:
            ...
            outfile = f'Attacks_{GenModel.__name__}_{tid}.pickle'
            file_path = path.join(f'{args.outdir}', f'{outfile}')
            if not os.path.exists(file_path):
                ...
                synA, labelsSA = generate_mia_shadow_data(GenModel, target, rawA, runconfig['sizeRawT'], runconfig['sizeSynT'], runconfig['nShadows'], runconfig['nSynA'], paramsList[0], runconfig['device'], dname)
                ...
                outfile = f'Attacks_{GenModel.__name__}_{tid}.pickle'
                with open(path.join(f'{args.outdir}', f'{outfile}'), 'wb') as file:
    ...
    outfile = f'Attacks_{GenModel.__name__}_{tid}.pickle'
    with open(path.join(f'{args.outdir}', f'{outfile}'), 'rb') as file:
        attacks = pickle.load(file)
    ...
    for nr in range(runconfig['nIter']):
        ...
        for GenModel in gmList:
            ...
            synTwithoutTarget = run_AEframework(rawTout, GenModel, paramsList[0][0], paramsList[0][1], n_samples=runconfig['sizeSynT'], n_samplesets=runconfig['nSynT'], device=runconfig['device'], dname=dname)
            synLabelsOut = [LABEL_OUT for _ in range(runconfig['nSynT'])]
            ...
            for tid in targetIDs:
                ...
                synTwithTarget = run_AEframework(rawTin, GenModel, paramsList[0][0], paramsList[0][1], n_samples=runconfig['sizeSynT'], n_samplesets=runconfig['nSynT'], device=runconfig['device'], dname=dname)
                synLabelsIn = [LABEL_IN for _ in range(runconfig['nSynT'])]
    ...
    resultsTargetPrivacy['time'] = elapsed_time_seconds/60

    outfile = f"ResultsMIA_{GenModel.__name__}_{tid}"

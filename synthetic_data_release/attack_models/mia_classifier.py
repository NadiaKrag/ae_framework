...
from synthetic_data_release.utils.datagen import convert_df_to_array
from synthetic_data_release.utils.utils import CustomProcess
from synthetic_data_release.utils.constants import *

from synthetic_data_release.attack_models.attack_model import PrivacyAttack
...
def generate_mia_shadow_data(GenModel, target, rawA, sizeRaw, sizeSyn, numModels, numCopies, paramsList, device, dname):
    ...
    if GenModel.multiprocess:
        ...
        for train_index, _ in kf.split(rawA):
            worker_train_shadow(rawA, train_index, GenModel, target, sizeSyn, numCopies, synA, labelsA, paramsList, device, dname)

    return synA, labelsA


def worker_train_shadow(rawA, train_index, GenModel, target, sizeSyn, numCopies, synA, labelsA, paramsList, device, dname):
    ...
    synOut = run_AEframework(rawAout, GenModel, paramsList[0], paramsList[1], n_samples=sizeSyn, n_samplesets=numCopies, device=device, dname=dname)
    labelsOut = [LABEL_OUT for _ in range(numCopies)]
    ...
    synIn = run_AEframework(rawAin, GenModel, paramsList[0], paramsList[1], n_samples=sizeSyn, n_samplesets=numCopies, device=device, dname=dname)
    labelsIn = [LABEL_IN for _ in range(numCopies)]

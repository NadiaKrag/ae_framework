...
def objective(trial):
    ...
    if args.tune_type == "cv":
        ...
        for fold in range(5):
            ...
            results_catboost.append(metrics_report.get_res())
        score /= 5
    ...
    result_path = f'tuned_models/catboost/{args.ds_name}_tune_results.json'
    if trial.number == 0:
        result_list = [stats_catboost]
        with open(result_path, 'w') as json_file:
            json.dump(result_list, json_file)
    else:
        with open(result_path, 'r') as json_file:
            result_list = json.load(json_file)
        result_list.extend([stats_catboost])
        with open(result_path, 'w') as json_file:
            json.dump(result_list, json_file)

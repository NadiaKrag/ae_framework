...
def get_results(results_dict_list, verbose=True):
    # Initialize a defaultdict with default values as lists
    metric_results = defaultdict(list)
    
    # Iterate through the results for each run
    for result_dict in results_dict_list:
        for phase, metrics in result_dict.items():
            for metric, value in metrics.items():
                metric_results[(phase, metric)].append(value)
    
    # Calculate mean and standard deviation for each metric
    metric_statistics = {}
    for (phase, metric), values in metric_results.items():
        mean_value = np.mean(values)
        std_deviation = np.std(values)
        metric_statistics[f'{phase}_{metric}'] = {'mean': mean_value, 'std_dev': std_deviation, 'data': values}
    
    # Print the results
    if verbose:
        for key, stats in metric_statistics.items():
            print(key, stats['mean'], stats['std_dev'])
            print(stats['data'])
